...
char rx_msg[UART_CHARS];
char rx_buf[1024 * UART_CHARS];
char pk_msg[2 * UART_CHARS];
uint8_t temp_msg[NUM_CHARS];
...
void int2bin(unsigned int in, int count, int* out) {
...
void bits2ints(int* bits, uint8_t* ints) {
    for (int i = 0; i < 6; i++) {
        ints[i] = 0;
        for (int j = 0; j < 8; j++) {
            ints[i] = 2 * ints[i] + bits[8*i+j];
        }
    }
}

void send_packets(char* rx_buf, int msg_end) {
    ...
    for (int i = 0; i < msg_end; i+=2) {
        int2bin(seq, 6, seq_control);
        char2bin(rx_buf[i], char1);
        char2bin(rx_buf[i+1], char2);
 
        int crc_check[38];
        memcpy(crc_check, src_node, 8*sizeof(int));
        memcpy(crc_check + 8, dest_node, 8*sizeof(int));
        memcpy(crc_check + 16, seq_control, 6*sizeof(int));
        memcpy(crc_check + 22, char1, 8*sizeof(int));
        memcpy(crc_check + 30, char2, 8*sizeof(int));

        int crc[3];
        crc3(crc_check, crc);

        int bits2send[48];
        memcpy(bits2send, size_and_parity, 7*sizeof(int));
        memcpy(bits2send + 7, crc_check, 38*sizeof(int));
        memcpy(bits2send + 45, crc, 3*sizeof(int));
        //printf("%c", rx_buf[i]);
        //printf("%c\n", rx_buf[i+1]);
        
        uint8_t message[6];
        bits2ints(bits2send, message);
        recalculate_output(message, NUM_CHARS);
...
void buffer_input(char c) {
    ...
    if (buf_index < 1024){
        if (c != '\r') {
            ...
            rx_buf[buf_index] = '\0';
            rx_buf[buf_index+1] = '\0';
            msg_end = buf_index;
            buf_index = 0;
//            send_packets(rx_buf, msg_end);
            temp_flag = 1;
        }
    } 
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	...
	HAL_UART_Transmit(&huart3, (uint8_t *)rx_msg, UART_CHARS, 1);
	//recalculate_output((uint8_t *) rx_msg, NUM_CHARS);
	buffer_input((char) rx_msg[0]);
}
